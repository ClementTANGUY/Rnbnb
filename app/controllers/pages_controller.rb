...
def search
      if params[:search].present? && params[:search].strip != ""
         session[:rnbnb_search] = params[:search]
      end

      arrResult = Array.new

      if session[:rnbnb_search] && session[:rnbnb_search] != ""
         @rooms_address = Room.where(active: true).near(session[:rnbnb_search], 5, order:'distance')
      else
