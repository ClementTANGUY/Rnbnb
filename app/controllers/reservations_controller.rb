  ...
  def create
    @reservation = current_user.reservations.create(reservation_params)
         ...
         if @reservation.save
                ...
                @payment = Payment.new({ email: User.find(@reservation.user_id).email, token: params[:payment]["token"], reservation_id: @reservation.id, amount: @reservation.total }) #on teste d’abord la création d’un paiement avec les renseignements de la réservation (email, prix total) et du champ carte bleue
